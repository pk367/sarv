...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            # Fetch the stock data (you can parameterize symbol, interval, etc.)
            symbol = "gold"
            exchange = "mcx"
            ...
            data = fetch_stock_data_and_resample(symbol, exchange, interval, INTERVAL_MAP[interval], n_bars, fut_contract)

            if data is not None and not data.empty:
                data_json = data.round(2).reset_index().to_dict(orient="records")
                # Send the data to the client via WebSocket
                await websocket.send_json({"symbol": symbol, "data": data_json})
            
            # Delay to control the frequency of updates (e.g., 5 seconds)
